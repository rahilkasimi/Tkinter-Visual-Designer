...
# Tuples are (light_mode_color, dark_mode_color)
TOOLBOX_NORMAL_COLOR = ("#FFFFFF", "#2B2B2B")
TOOLBOX_HOVER_COLOR = ("#E3F2FD", "#1E3A5F")
TOOLBOX_ACTIVE_COLOR = ("#FF6B35", "#FF6B35")
...
class GUIBuilderApp:
    ...
    def _setup_styles(self):
        """Configure default font and the only remaining ttk styles (Treeview).

        CustomTkinter styles all migrated widgets natively, so the previous ttk
        rules for TFrame/TLabel/TButton/TEntry/TCombobox/TScrollbar/TPanedWindow/
        Toolbox*/PropRow/Toolbar/Status/TNotebook are no longer required. Only the
        Treeview styles are kept because CustomTkinter has no Treeview equivalent
        (used by the Table element rendered on the drawing canvas).
        """
        style = ttk.Style()
        ...
        # Color palette (retained for Treeview styling only)
        bg = "#F5F5F5"
        fg = "#212121"
        select_bg = "#1976D2"

        # Treeview (Table) — no CTk equivalent, keep ttk styling
    ...
    def _build_ui(self):
        ...
        self.toolbox_frame = ctk.CTkFrame(self.main_paned, width=220, corner_radius=0)
        ...
        self.prop_frame = ctk.CTkFrame(self.main_paned, width=300, corner_radius=0)
        ...
        ctk.CTkLabel(code_header, text="LIVE CODE", font=ctk.CTkFont(size=10, weight="bold")).pack(side=tk.LEFT, padx=5)

        self.code_text = ctk.CTkTextbox(
            self.code_frame, font=("Consolas", 9), wrap="none",
        ...
        ctk.CTkLabel(status_bar, textvariable=self.status_var, anchor="w").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4, pady=2)
        ctk.CTkLabel(status_bar, textvariable=self.count_var, anchor="e").pack(side=tk.RIGHT, padx=4, pady=2)
    ...
    def _build_toolbar(self):
        ...
        def _sep():
            ctk.CTkFrame(toolbar, width=2, height=28).pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=2)

        ctk.CTkButton(toolbar, text="📄 New Design", command=self._new_design, width=126, height=28).pack(side=tk.LEFT, padx=2)
        ctk.CTkButton(toolbar, text="📂 Load Design", command=self._load_design, width=134, height=28).pack(side=tk.LEFT, padx=2)
        ctk.CTkButton(toolbar, text="💾 Save Design", command=self._save_design, width=134, height=28).pack(side=tk.LEFT, padx=2)
        _sep()
        ctk.CTkButton(toolbar, text="↶ Undo", command=self._undo, width=75, height=28).pack(side=tk.LEFT, padx=2)
        ctk.CTkButton(toolbar, text="↷ Redo", command=self._redo, width=75, height=28).pack(side=tk.LEFT, padx=2)
        _sep()
        ctk.CTkButton(toolbar, text="🗑️ Delete", command=self._delete_selected, width=100, height=28).pack(side=tk.LEFT, padx=2)
        ctk.CTkButton(toolbar, text="🧹 Clear Canvas", command=self._clear_all, width=143, height=28).pack(side=tk.LEFT, padx=2)
        _sep()

        ctk.CTkButton(toolbar, text="📋 Copy Code", command=self._copy_code, width=117, height=28).pack(side=tk.LEFT, padx=2)
        ctk.CTkButton(toolbar, text="▶ Run Preview", command=self._run_preview, width=134, height=28).pack(side=tk.LEFT, padx=2)
        _sep()
        ctk.CTkButton(toolbar, text="👁️ Toggle Code", command=self._toggle_code_view, width=143, height=28).pack(side=tk.LEFT, padx=2)
    ...
    def _build_toolbox(self):
        ctk.CTkLabel(self.toolbox_frame, text="TOOLBOX", font=ctk.CTkFont(size=10, weight="bold")).pack(pady=(5, 10))

        scrollable_frame = ctk.CTkScrollableFrame(self.toolbox_frame, fg_color="transparent")
        scrollable_frame.pack(fill=tk.BOTH, expand=True, padx=2, pady=2)
        ...
        for cat in sorted(categories.keys()):
            ctk.CTkLabel(scrollable_frame, text=cat, font=ctk.CTkFont(size=9, weight="bold"), anchor="w").pack(anchor=tk.W, padx=5, pady=(10, 2))
            for name, spec in sorted(categories[cat], key=lambda x: x[0]):
                ...
                item_frame = ctk.CTkFrame(scrollable_frame, cursor="hand2", corner_radius=6, fg_color=TOOLBOX_NORMAL_COLOR)
                item_frame.pack(fill=tk.X, padx=5, pady=2)

                lbl_icon = ctk.CTkLabel(item_frame, text=icon, anchor="w")
                lbl_icon.pack(side=tk.LEFT, padx=6, pady=4)

                lbl_name = ctk.CTkLabel(item_frame, text=elem_name, anchor="e")
    ...
    def _build_property_inspector(self):
        ctk.CTkLabel(self.prop_frame, text="PROPERTIES", font=ctk.CTkFont(size=10, weight="bold")).pack(pady=(5, 6))
        self.prop_title_label = ctk.CTkLabel(self.prop_frame, text="No element selected.", anchor="w")
        self.prop_title_label.pack(anchor=tk.W, padx=6, pady=(0, 2), fill=tk.X)
        self.prop_context_var = tk.StringVar(value="Container: None")
        ctk.CTkLabel(self.prop_frame, textvariable=self.prop_context_var, text_color="#757575", anchor="w").pack(anchor=tk.W, padx=6, pady=(0, 5), fill=tk.X)

        self.prop_scrollable = ctk.CTkScrollableFrame(self.prop_frame, width=270, fg_color="transparent")
        self.prop_scrollable.pack(fill=tk.BOTH, expand=True)

        self.prop_rows = []
        for i in range(20):
            frame = ctk.CTkFrame(self.prop_scrollable, corner_radius=0, fg_color="transparent")
            lbl = ctk.CTkLabel(frame, text="", width=90, anchor="w")
            lbl.pack(side=tk.LEFT, padx=(2, 4))
            control_frame = ctk.CTkFrame(frame, fg_color="transparent", corner_radius=0)
    ...
    def _show_properties_multi(self):
        for row in self.prop_rows:
            ...
            row["visible"] = False
        self.prop_title_label.configure(text=f"[{len(self.selected_elems)} elements selected - Common Properties]")
        ...
        for field_key, label, widget_type in common_fields:
            ...
            if widget_type in ("entry", "color"):
                ...
                if widget_type == "entry": ctk.CTkEntry(row["control_frame"], textvariable=var, width=200).pack(fill=tk.X)
                elif widget_type == "color":
                    frame = ctk.CTkFrame(row["control_frame"], fg_color="transparent", corner_radius=0)
                    frame.pack(fill=tk.X)
                    ctk.CTkEntry(frame, textvariable=var, width=200).pack(side=tk.LEFT, fill=tk.X, expand=True)
                    ctk.CTkButton(frame, text="Pick", command=lambda v=var: self._pick_color(v), width=60, height=28).pack(side=tk.RIGHT)
                ...
                ctk.CTkComboBox(frame, variable=family_var, values=families, width=200).pack(side=tk.LEFT, padx=(0, 2))
                ctk.CTkComboBox(frame, variable=size_var, values=[str(s) for s in [8,9,10,11,12,14,16,18,20,24]], width=200).pack(side=tk.LEFT)
    ...
    def _show_properties(self, elem: Optional[DesignElement]):
        ...
        self.prop_title_label.configure(text=f"{spec['display']} [id={elem.elem_id}]")
        ...
        for fielddef in fields:
            ...
            if elem.elem_type == "Table" and field_key == "file":
                ...
                row["var"] = var
                file_frame = ctk.CTkFrame(row["control_frame"], fg_color="transparent", corner_radius=0)
                file_frame.pack(fill=tk.X)
                ctk.CTkEntry(file_frame, textvariable=var, width=200).pack(side=tk.LEFT, fill=tk.X, expand=True)
                ctk.CTkButton(file_frame, text="…", width=60, height=28, command=lambda v=var: self._browse_table_file(v)).pack(side=tk.LEFT, padx=(3, 0))
                row["frame"].pack(fill=tk.X, pady=2)
                row["visible"] = True
                row_index += 1
                continue
            ...
            if field_key == "tabs" and elem.elem_type == "Notebook":
                var.trace_add("write", lambda *args, r=row: self._on_live_prop_change(r))
                row["var"] = var
                tabs_frame = ctk.CTkFrame(row["control_frame"], fg_color="transparent", corner_radius=0)
                tabs_frame.pack(fill=tk.X)
                ctk.CTkEntry(tabs_frame, textvariable=var, width=200).pack(side=tk.LEFT, fill=tk.X, expand=True)
                ctk.CTkButton(tabs_frame, text="+", width=60, height=28, command=lambda e=elem: self._add_notebook_tab(e)).pack(side=tk.LEFT, padx=(3, 0))
                ctk.CTkButton(tabs_frame, text="−", width=60, height=28, command=lambda e=elem: self._remove_notebook_tab(e)).pack(side=tk.LEFT, padx=(2, 0))
            elif widget_type in ("entry", "combobox", "color"):
                var.trace_add("write", lambda *args, r=row: self._on_live_prop_change(r))
                row["var"] = var
                if widget_type == "entry":
                    ctk.CTkEntry(row["control_frame"], textvariable=var, width=200).pack(fill=tk.X)
                elif widget_type == "combobox":
                    ctk.CTkComboBox(row["control_frame"], variable=var, values=[str(o) for o in (options or [])], width=200).pack(fill=tk.X)
                else:
                    cf = ctk.CTkFrame(row["control_frame"], fg_color="transparent", corner_radius=0)
                    cf.pack(fill=tk.X)
                    ctk.CTkEntry(cf, textvariable=var, width=200).pack(side=tk.LEFT, fill=tk.X, expand=True)
                    ctk.CTkButton(cf, text="Pick", width=60, height=28, command=lambda v=var: self._pick_color(v)).pack(side=tk.RIGHT, padx=(3, 0))
                ...
                ctk.CTkComboBox(frame, variable=family_var, values=families, width=130, state="readonly").pack(side=tk.LEFT, padx=(0, 2))
                ctk.CTkComboBox(frame, variable=size_var, values=[str(s) for s in [8,9,10,11,12,14,16,18,20,24,28,36,48]], width=70).pack(side=tk.LEFT)
                var.trace_add("write", lambda *args, r=row: self._on_live_prop_change(r))
                row["var"] = var
            row["frame"].pack(fill=tk.X, pady=2)
            row["visible"] = True
    ...
    def _show_canvas_properties(self):
        ...
        title_entry = ctk.CTkEntry(row["control_frame"], textvariable=self.title_var)
        ...
        ctk.CTkEntry(row["control_frame"], textvariable=var_w).pack(fill=tk.X)
        ...
        ctk.CTkEntry(row["control_frame"], textvariable=var_h).pack(fill=tk.X)
        ...
        frame_bg = ctk.CTkFrame(row["control_frame"], fg_color="transparent")
        frame_bg.pack(fill=tk.X)
        ctk.CTkEntry(frame_bg, textvariable=var_bg, width=100).pack(side=tk.LEFT, fill=tk.X, expand=True)
        ctk.CTkButton(frame_bg, text="Pick", width=60, height=28, command=lambda v=var_bg: self._pick_color(v)).pack(side=tk.RIGHT, padx=(4, 0))
    ...
    def _open_code_editor(self, elem: DesignElement):
        ...
        text_widget.tag_config("syntax_error", background="#FF4444", foreground="white")
        ...
        # --- Syntax checker status bar (below editor) ---
        ...
        syntax_status_label = ctk.CTkLabel(syntax_bar, textvariable=syntax_status_var, anchor="w")
        syntax_status_label.pack(side=tk.LEFT, padx=8)
        line_col_label = ctk.CTkLabel(syntax_bar, textvariable=line_col_var, anchor="e")
        ...
        def _check_syntax():
            ...
            try:
                ...
                syntax_status_label.configure(text_color=("#1B7F3B", "#4ADE80"))
                return True
            except SyntaxError as e:
                lineno = e.lineno or 1
                msg = e.msg or "invalid syntax"
                syntax_status_var.set(f"✗ Syntax error (line {lineno}): {msg}")
                syntax_status_label.configure(text_color=("#C62828", "#FF6B6B"))
                try:
                    text_widget.tag_add("syntax_error", f"{lineno}.0", f"{lineno}.end")
                except Exception:
                    pass
                return False
        ...
        ctk.CTkButton(btn_frame, text="💾 Save", command=save_code, width=100, height=28).pack(side=tk.LEFT, padx=2)
        ctk.CTkButton(btn_frame, text="💻 Open in VS Code", command=open_in_vscode, width=150, height=28).pack(side=tk.LEFT, padx=2)
        ctk.CTkButton(btn_frame, text="Close", command=top.destroy, width=80, height=28).pack(side=tk.RIGHT, padx=2)
        text_widget.bind("<KeyRelease>", lambda event: (_schedule_check(), _update_line_col()), add="+")
